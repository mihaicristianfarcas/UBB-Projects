...
public:
    ...
    explicit Repository(int capacity);

    ~Repository();

    void addElement(const T& element);

    void removeElement(const std::string &name); // Assuming name is unique
...
template <typename T>
int Repository<T>::getLength() const {
    return this->size;
}

/// Get the dog positioned at a specific index
template <typename T>
T& Repository<T>::getElementAt(int index) {
    return this->elements[index];
}

/// Get all elements in the vector
template <typename T>
T* Repository<T>::getAllElements() const {
