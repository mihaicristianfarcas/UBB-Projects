package com.ordersProducts.util;
    ...
    static {
        ...
        try (InputStream input = DatabaseUtil.class.getClassLoader().getResourceAsStream("database.properties")) {
            if (input == null) {
                ...
                DB_URL = "jdbc:mysql://localhost:3306/ordersProducts";
                DB_USER = "root";
                DB_PASSWORD = "";
            } else {
                props.load(input);
                DB_URL = props.getProperty("db.url", "jdbc:mysql://localhost:3306/ordersProducts");
