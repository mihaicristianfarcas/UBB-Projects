...
def pollard_rho(n, f=None):
    ...
    while True:
        ...
        # Add row to table
        table_data.append(
            [f"x_{i-1} = {x_odd}", f"x_{i} = {x_even}", f"(|x_{i} - x_{j}|, n) = {d}"])
        ...
        if d == n:
            print(tabulate(table_data, tablefmt="simple_grid"))
            raise RuntimeError(
                f"Algorithm failed: gcd = n. Try a different polynomial or starting value.")
        ...
        if i > 10**6:
            print(tabulate(table_data, tablefmt="simple_grid"))
            raise RuntimeError(
                f"Algorithm did not converge after {i} iterations")
...
def main():
    ...
    while True:
        choice = input(
            "Choose a polynomial function (1-5, or press Enter for default): ").strip()
