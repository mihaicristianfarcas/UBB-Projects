...
def verify_block_size(k, n):
    ...
    """
    if 27 ** k >= n:
        raise ValueError(f"Block size k={k} is too large for n={
                         n}. Must satisfy 27^k < n.")
    return True
...
def main():
    ...
    if is_encrypt:
        ...
        if len(padded_message) != len(message):
            print(f"Padded message: '{padded_message.upper().replace(
                ' ', '_')}' (padded to multiple of {k})")
        ...
        if message_blocks:
            ...
            for i, char in enumerate(first_block_text):
                ...
                print(f"  Step {i+1}: {result // 27 if i >
                      0 else 0} × 27 + {digit} = {result}")
        ...
        print(f"\nFormatted ciphertext (length {l}): {
              ' '.join(formatted_cipher)}")
        ...
        if len(padded_ciphertext) != len(ciphertext_input):
            print(f"Padded ciphertext: '{padded_ciphertext.upper().replace(
                ' ', '_')}' (padded to multiple of {l})")
