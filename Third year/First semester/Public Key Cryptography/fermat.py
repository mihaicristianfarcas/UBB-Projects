...
def fermat_factorization(n, B=None):
    ...
    for i in range(1, B + 1):
        ...
        if is_square:
            ...
            print(
                f"✓ Therefore: n = (t - s)(t + s) = ({t} - {s})({t} + {s}) = {a} × {b}")
    ...
    print(tabulate(table_data,
                   headers=["t", "t²", "t² - n", "Is square?"],
                   tablefmt="simple_grid"))
    raise RuntimeError(f"Algorithm failed: no factors found within bound B = {
                       B}. Try increasing B.")
...
def main():
    ...
    while True:
        try:
            ...
            is_square, sqrt_n = is_perfect_square(n)
            if is_square:
                print(f"Error: n is a perfect square ({
                      sqrt_n}²). Please try again.")
                continue
    ...
    while True:
        bound_input = input(
            f"Enter bound B (press Enter for default = {n}): ").strip()
