...
std::vector<Polynomial::coeff_t> compute_naive_chunk(const Polynomial& a, const Polynomial& b, std::size_t start, std::size_t end, std::size_t result_size) {
...
Polynomial naive_parallel(const Polynomial& a, const Polynomial& b, int threads) {
    ...
    for (std::size_t start = 0; start < a.size(); start += chunk) {
        ...
        futures.emplace_back(std::async(std::launch::async, compute_naive_chunk, std::cref(a), std::cref(b), start, end, result_size));
    }
