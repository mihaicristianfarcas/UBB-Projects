...
from automata import (
    build_identifier_fa,
    build_integer_constant_fa,
    build_string_constant_fa,
)
...
class LexicalAnalyzer:
    ...
    def __init__(self, token_codes: dict, lib: SymbolTableLibrary):
        ...
        self.identifier_fa = build_identifier_fa()
        self.integer_constant_fa = build_integer_constant_fa()
        self.string_constant_fa = build_string_constant_fa()
        ...
        for line_number, line in enumerate(lines, start=1):
            ...
            while index < length:
                ...
                if current_char == '#':
                    break
                segment = line[index:]
                match_lexeme, token_type = self._match_token(segment)
                    ...
                    if current_char == '"':
                        errors.append(
                            LexicalError(
                                ...
                                index + 1,
                                segment,
                                "Invalid string literal (missing closing quote or contains disallowed characters)",
                            )
                        )
                        break
                    errors.append(
    ...
    def _match_token(self, segment: str) -> Tuple[Optional[str], Optional[str]]:
        lexeme, token_type = self._match_with_automata(segment)
        ...
        for candidate_type, pattern in self.TOKEN_PATTERNS:
            ...
            if match:
                return match.group(0), candidate_type

        return None, None

    def _match_with_automata(self, segment: str) -> Tuple[Optional[str], Optional[str]]:
        string_length = self.string_constant_fa.longest_prefix(segment)
        if string_length:
            return segment[:string_length], "STRING"

        number_length = self.integer_constant_fa.longest_prefix(segment)
        if number_length:
            return segment[:number_length], "NUMBER"

        identifier_length = self.identifier_fa.longest_prefix(segment)
        if identifier_length:
            return segment[:identifier_length], "IDENTIFIER"
